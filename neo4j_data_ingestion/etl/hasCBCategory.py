"""Funding Module for hasCBCategory relation in neo4j"""
...
log_file = 'hasCBCategory.py'

class HasCBCategory(Database):
	"""Creates and updates hasCBCategory relation in neo4j."""
	...
	def __init__(self):
		...
		self.unique_keys = ['Name','Permalink','PrimaryRole']
		self.skip = []
	...
	def create_update_hasCBCategory_relationship(self, data):
			...
			for row in data:
				try:
					...
					self.logg(debug_msg = 'Error while creating neo4j node query.', 
						info_msg = 'Function = create_update_hasCBCategory_relationship()', 
	...
	def transform_data(self, data):
		...
		for row in data:
			...
			PrimaryRole = row.get('PrimaryRole')
	...
	def run(cls):
		...
		try:
			hasCBCategory = cls()
			data = hasCBCategory.get_data(data_set='all')
			if data:
				updated_data = hasCBCategory.transform_data(data)
				if updated_data:
					hasCBCategory.create_update_hasCBCategory_relationship(updated_data)
