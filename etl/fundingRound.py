...
class FundingRound(Database):
	...
	def initialize_variables(self):
		...
		self.neo4jQuery = self.get_neo4j_queries('FundingRound', 'merge', 'node')
	...
	def get_data(self, data_set, funding_round_id=None):
		...
		if data_set == 'delta':
			data = self.fetch_all_mysql_data(self.date, 'FundingRound', 'select', 'node')
			return data

		elif data_set == 'on_demand':
			data = self.fetch_all_mysql_data(funding_round_id, 'FundingRound', 'select_ondemand', 'node')
			return data
	...
	def run(cls, data_set='delta'):
		...
		fundingRound = cls()
		data = fundingRound.get_data(data_set)
